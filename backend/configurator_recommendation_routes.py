    ...
    async def recommend_variants(request: AIRecommendationRequest) -> AIRecommendationResponse:
        ...
        if not variants:
            return AIRecommendationResponse(
                recommendations=[],
                explanation="No active vehicle variants are available for this request.",
                ai_assisted=False,
                valid=True,
            )

        variant_ids = [str(item.get("variant_id")) for item in variants if item.get("variant_id")]
        pricing_rows = await db.variant_pricing.find(
        ...
        for item in ranked:
            ...
            status = candidate.get("configurator_status", "COMING_SOON")
            configurator_available = status == "AVAILABLE" and bool(candidate.get("configurator_asset_id"))
            ...
            recommendations.append({
                **item,
                "availability_status": str(status),
        ...
        if not recommendations:
            ...
            explanation = "Recommendations are ranked deterministically from active backend catalog data; pricing, availability and configurator readiness remain backend-authoritative."
        return AIRecommendationResponse(
