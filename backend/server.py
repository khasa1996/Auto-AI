...
async def ai_chat(req: ChatRequest, request: Request, caller_phone: str = Depends(current_user_phone)):
    if not await _rate_allowed("ai-chat-user", caller_phone, 30, 300) or not await _rate_allowed("ai-chat-ip", _client_ip(request), 60, 300):
    ...
    try:
        ...
        if (
            existing_session
            and existing_session.get("phone") != caller_phone
        ):
            raise HTTPException(
                status_code=404,
...
async def create_booking(req: BookingRequest, request: Request):
    if not await _rate_allowed("booking-ip", _client_ip(request), 60, 600):
    ...
    if existing:
        if existing.get("phone") != req.phone:
            raise HTTPException(
                status_code=409,
                detail="Idempotency key is already in use",
            )
        return Booking.model_validate(existing)
    ...
    leads = _assign_partners_to_booking(
        booking,
        car,
    )
    if leads:
...
async def send_otp(req: OtpSendReq, request: Request):
    if not await _rate_allowed("otp-send-phone", req.phone, 5, 600) or not await _rate_allowed("otp-send-ip", _client_ip(request), 40, 600):
        raise HTTPException(status_code=429, detail="Too many OTP requests, try again later")
    otp = security.generate_otp(); await db.otps.delete_many({"phone":req.phone}); await db.otps.insert_one({"phone":req.phone,"otp_hash":security.hash_secret(otp),"attempts":0,"expires_at":security.expiry_datetime(seconds=security.OTP_TTL_SECONDS),"ts":datetime.now(timezone.utc)})
...
async def verify_otp(req: OtpVerifyReq, request: Request):
    if not await _rate_allowed("otp-verify-phone", req.phone, 10, 600) or not await _rate_allowed("otp-verify-ip", _client_ip(request), 60, 600):
    ...
    claimed = await db.otps.find_one_and_update(
        {
            "phone": req.phone,
            "expires_at": {
    ...
    await db.user_sessions.insert_one(
        {
            ...
            "phone": req.phone,
            "expires_at": security.expiry_datetime(
                hours=security.USER_SESSION_TTL_HOURS
            ),
...
async def dealer_apply(app: DealerApplication, request: Request):
    if not await _rate_allowed("dealer-apply-ip", _client_ip(request), 20, 600):
...
async def admin_verify(req: AdminPinReq, request: Request):
    if not await _rate_allowed("admin-login-ip", _client_ip(request), 5, 900):
        raise HTTPException(status_code=429,detail="Too many attempts, try again later")
    if not _admin_pin_valid(req.pin): raise HTTPException(status_code=401,detail="Invalid admin PIN")
    token=security.generate_token(); await db.admin_sessions.insert_one({"token_hash":security.hash_secret(token),"expires_at":security.expiry_datetime(hours=security.ADMIN_SESSION_TTL_HOURS),"ts":datetime.now(timezone.utc)}); return {"ok":True,"token":token}
...
async def create_checkout_order(
    ...
    if existing:
        if (
            existing.get("phone") != customer_phone
            or existing.get("plan_id") != req.plan_id
        ):
            raise HTTPException(
                status_code=409,
                detail="Idempotency key is already in use",
            )
        if existing.get("payment_status") == "creating":
            raise HTTPException(
                status_code=409,
                detail="Payment order creation is already in progress",
            )
        plan = PLANS[existing["plan_id"]]
        return {
    ...
    reservation = {
        "id": str(uuid.uuid4()),
        "idempotency_key": req.idempotency_key,
        "order_id": f"pending_{uuid.uuid4().hex}",
        "receipt": receipt,
        "plan_id": req.plan_id,
        "amount": plan["amount"],
    ...
    try:
        await db.payment_transactions.insert_one(reservation)
    except DuplicateKeyError:
        existing = await db.payment_transactions.find_one(
            {
                "idempotency_key": req.idempotency_key,
        ...
        if (
            not existing
            or existing.get("phone") != customer_phone
            or existing.get("plan_id") != req.plan_id
        ):
            raise HTTPException(
                status_code=409,
...
async def verify_checkout(req: CheckoutVerifyRequest, caller_phone: str=Depends(current_user_phone)):
    ...
    if tx.get("payment_status") == "paid":
        ...
        if not payment_id:
            raise HTTPException(
                status_code=409,
                detail="Paid transaction is missing payment confirmation",
            )
    ...
    if str(payment.get("currency", "")).upper() != str(
        tx["currency"]
    ).upper():
        raise HTTPException(
            status_code=400,
...
async def razorpay_webhook(request: Request):
    ...
    if (
        ...
        await db.processed_payment_events.delete_one(
            {"_id": event_id}
        )
        return {"ok": True}
    ...
    try:
        ...
        if not tx:
            logger.warning(
                "Ignoring payment webhook for unknown order=%s",
                order_id,
            )
            return {"ok": True}

        if (
            int(payment_entity.get("amount", 0))
            != int(tx["amount_paise"])
        ):
            logger.error(
                "Payment webhook amount mismatch order=%s",
                order_id,
            )
            return {"ok": True}

        if (
            str(payment_entity.get("currency", "")).upper()
            != str(tx["currency"]).upper()
        ):
            logger.error(
                "Payment webhook currency mismatch order=%s",
                order_id,
            )
            return {"ok": True}
        ...
        logger.exception(
            "Razorpay webhook processing failed order=%s",
            order_id,
        )
        raise HTTPException(
            status_code=503,
...
async def my_subscription(
    phone: str = Depends(current_user_phone),
):
    entitlements = await db.entitlements.find(
        {
            "phone": phone,
...
app.add_middleware(CORSMiddleware, allow_credentials=ALLOW_CREDENTIALS, allow_origins=CORS_ORIGINS, allow_methods=["GET","POST","OPTIONS"], allow_headers=["Authorization","Content-Type","X-Request-ID"])
