...
function ConfiguratorScene({ modelUrl, paintColorHex, paintMaterialNames, wheelMeshNames, optionMeshNames, purchasable, interaction, sceneRef }) {
  useCameraPreset(interaction.cameraPreset, null);
  ...
  return <>
    ...
    <ConfiguratorControls autoRotate={interaction.autoRotate} onInteract={() => useConfiguratorStore.getState().pauseAutoRotate()} />
  </>;
}
...
export default function ConfiguratorViewer({ style, options }) {
  ...
  const capture = async (share = false) => {
    const canvas = document.querySelector('.auto-ai-configurator-canvas canvas');
    if (!canvas) return;
