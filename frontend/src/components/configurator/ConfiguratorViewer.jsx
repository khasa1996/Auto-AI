/**
 * ConfiguratorViewer — React Three Fiber scene for the real 3D configurator.
 *
 * The viewer only renders verified GLB/GLTF assets supplied by the backend.
 * Purchasable selections remain separate from showroom interactions.
 */
  ...
  return (
    <>
      ...
      <ConfiguratorControls
        ...
        onInteract={() => useConfiguratorStore.getState().pauseAutoRotate()}
...
export default function ConfiguratorViewer({ style, options }) {
  ...
  const selectedPaint = options?.colors?.find(
    (color) => color.color_id === purchasable.paintId,
  );
  const paintColorHex = selectedPaint?.primary_hex || null;
